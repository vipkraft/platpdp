...
uses
  ...
  ExtCtrls, Buttons, IniFiles, Menus, EditBtn, LCLPROC;
...
type
  ...
  public
    ...
    class function SetSite(sitename:string):boolean;
...
begin
  if not btnConnect.Enabled then begin
    ...
    txtSite.Text := 'z-it';
    txtPass.Text := '5HWF90hrB5';
    txtHost.Text := '62.105.41.108';
    txtPort.Text := '31004';
    txtUser.Text := 'oaoavtovokzal';
    txtPath.Text := '/';
    txtldir.text := '';
...
begin
  ...
  try
    ...
    for i := 0 to High(FSites) do begin
      ...
      ini.WriteString(s, 'pass', FSites[i].Pass);
...
class function TfrmSites.SetSite(sitename:string):boolean;
var
  Ini: TIniFile;
  i,n: Integer;
  s: string;
begin
  ...
  try
    ...
    nident := Ini.ReadString('global','ident', '22013');
    nOkato := Ini.ReadString('global','okato', '07701000');
    for n:=1 to I do
    begin
    s := 'site'+IntToStr(n);
